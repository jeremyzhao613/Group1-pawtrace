    ...
    let aiCollarContextPreviewEl = null;
    let aiCollarSyncBtn = null;
    let aiModuleStatusEl = null;
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // NFC public card page:
      // /nfc?userId=...&petId=...
      const nfcCardPage = document.getElementById('nfc-card-page');
      if (nfcCardPage) {
        const url = new URL(window.location.href);
        const isNfcCardRoute = url.pathname.endsWith('/nfc') || url.pathname.endsWith('/nfc.html');
        ...
        if (isNfcCardRoute) {
          ...
          nfcCardPage.classList.remove('hidden');

          const statusEl = document.getElementById('nfc-status');
          const petAvatarEl = document.getElementById('nfc-pet-avatar');
          const petAvatarPlaceholderEl = document.getElementById('nfc-pet-avatar-placeholder');
          const petNameEl = document.getElementById('nfc-pet-name');
          const petMetaEl = document.getElementById('nfc-pet-meta');
          const ownerNameEl = document.getElementById('nfc-owner-name');
          const ownerContactEl = document.getElementById('nfc-owner-contact');
          const ownerCampusEl = document.getElementById('nfc-owner-campus');
          const petHealthEl = document.getElementById('nfc-pet-health');
          const petStatusEl = document.getElementById('nfc-pet-status');
          const contactLinkEl = document.getElementById('nfc-contact-link');
          const closeBtn = document.getElementById('nfc-close-btn');
          ...
          if (!userId || !petId) {
            ...
            statusEl.textContent = 'Missing userId / petId in URL.';
            return;
          }

          statusEl?.classList.remove('hidden');
          statusEl.textContent = 'Loading public card...';

          fetch(`/api/nfc/public?userId=${encodeURIComponent(userId)}&petId=${encodeURIComponent(petId)}`)
            .then(async (r) => {
              if (!r.ok) {
            ...
            .then((data) => {
              ...
              if (!pet || !user) throw new Error('Card data missing');

              petNameEl.textContent = pet.name || 'Unknown pet';
              const metaBits = [
                pet.type ? `${pet.type}` : '',
                pet.breed ? `${pet.breed}` : '',
                pet.age ? `Age: ${pet.age}` : '',
              ...
              petMetaEl.textContent = metaBits.join(' · ');

              ownerNameEl.textContent = user.displayName || user.username || 'Unknown';
              ownerContactEl.textContent = user.contact || 'N/A';
              ownerCampusEl.textContent = user.campus || '';

              petHealthEl.textContent = pet.health || 'No health notes';
              petStatusEl.textContent = pet.status ? `Status: ${pet.status}` : '';

              if (pet.avatar) {
                petAvatarEl.src = pet.avatar;
                petAvatarEl.classList.remove('hidden');
                petAvatarPlaceholderEl?.classList.add('hidden');
              } else {
                petAvatarEl.classList.add('hidden');
                petAvatarPlaceholderEl?.classList.remove('hidden');
              }

              // Best-effort: if contact looks like an email/phone, set a clickable link.
              const contact = String(user.contact || '');
              if (contact.includes('@')) {
                contactLinkEl.href = `mailto:${contact}`;
                contactLinkEl.classList.remove('hidden');
              } else if (/\\d{3,}/.test(contact)) {
                // naive phone handling; keep it simple for course demo
                contactLinkEl.href = `tel:${contact.replace(/\\s+/g, '')}`;
                contactLinkEl.classList.remove('hidden');
              }
      ...
      aiCollarContextPreviewEl = document.getElementById('ai-collar-context-preview');
      aiCollarSyncBtn = document.getElementById('btn-ai-collar-sync');
      aiModuleStatusEl = document.getElementById('ai-module-status');
      ...
      // --- 3D realtime pet map (course demo)
      // Uses Three.js plane + sprites; polls backend for latest coords.
      const btn3d = document.getElementById('btn-map-3d-toggle');
      ...
      const map3dCanvas = document.getElementById('map-3d-canvas');
      ...
      function setMap3dEnabled(enabled) {
        map3dEnabled = Boolean(enabled);
        if (map3dCanvas) map3dCanvas.classList.toggle('hidden', !map3dEnabled);
        mapBg?.classList.toggle('hidden', map3dEnabled);
        mapMarkersLayer?.classList.toggle('hidden', map3dEnabled);
        mapPetsLayer?.classList.toggle('hidden', map3dEnabled);
        if (btn3d) btn3d.classList.toggle('bg-primary', map3dEnabled);
      }

      function create3dMap() {
        if (!map3dCanvas || !window.THREE) return null;
        const THREE = window.THREE;
        const renderer = new THREE.WebGLRenderer({ canvas: map3dCanvas, antialias: true, alpha: true });
        ...
        const planeGeo = new THREE.PlaneGeometry(2.4, 1.6, 1, 1);
        const tex = new THREE.TextureLoader().load('/assets/m1.jpg');
        tex.colorSpace = THREE.SRGBColorSpace;
        const planeMat = new THREE.MeshStandardMaterial({ map: tex, roughness: 0.95, metalness: 0.0 });
        const plane = new THREE.Mesh(planeGeo, planeMat);
        plane.rotation.x = -Math.PI / 2.6;
        plane.position.y = 0;
        group.add(plane);

        const petDots = new Map(); // key -> mesh
        const dotGeo = new THREE.SphereGeometry(0.03, 18, 18);
        const colors = [0x99cdd8, 0xf59e0b, 0x34d399, 0xfb7185, 0x60a5fa];

        // Bounds (calibration): can be adjusted via UI and stored in localStorage.
        const BOUNDS_LS_KEY = 'pawtraceMapBoundsV1';
        const defaultBounds = { minLat: 31.0, maxLat: 32.0, minLon: 120.0, maxLon: 122.0 };
        function loadBounds() {
          try {
            const raw = localStorage.getItem(BOUNDS_LS_KEY);
            ...
            const out = {
              ...
              minLon: Number(b.minLon),
              maxLon: Number(b.maxLon),
            };
            if (![out.minLat, out.maxLat, out.minLon, out.maxLon].every(Number.isFinite)) return defaultBounds;
            if (out.minLat >= out.maxLat || out.minLon >= out.maxLon) return defaultBounds;
            return out;
          } catch {
        ...
        function saveBounds(b) {
          localStorage.setItem(BOUNDS_LS_KEY, JSON.stringify(b));
        }
        let bounds = loadBounds();

        function latLonToPlane(lat, lon) {
          const xN = (lon - bounds.minLon) / (bounds.maxLon - bounds.minLon);
          const zN = 1 - (lat - bounds.minLat) / (bounds.maxLat - bounds.minLat);
          const x = (xN - 0.5) * 2.2;
          const z = (zN - 0.5) * 1.35;
          return { x, z };
        }

        function upsertPet(key, lat, lon, label) {
          if (!Number.isFinite(lat) || !Number.isFinite(lon)) return;
          let mesh = petDots.get(key);
          if (!mesh) {
            const mat = new THREE.MeshStandardMaterial({ color: colors[petDots.size % colors.length] });
            mesh = new THREE.Mesh(dotGeo, mat);
            mesh.position.y = 0.08;
            mesh.userData = { label: label || key };
            group.add(mesh);
            petDots.set(key, mesh);
          }
          const pos = latLonToPlane(lat, lon);
          mesh.position.x = pos.x;
          mesh.position.z = pos.z;
        }

        function resize() {
          const rect = map3dCanvas.getBoundingClientRect();
        ...
        async function poll() {
          ...
          try {
            ...
            // Prefer health measurements (already fused lat/lon), fallback to location points.
            const healthUrl = userKey
              ? `/api/health/measurements?userId=${encodeURIComponent(userKey)}&limit=50`
              : `/api/health/measurements?limit=50`;
            const hrRes = await fetch(healthUrl, { cache: 'no-store' });
            if (hrRes.ok) {
              const data = await hrRes.json();
              const items = Array.isArray(data.items) ? data.items : [];
              const latestByKey = {};
              items.forEach(m => {
                const k = m.userId || m.deviceId || 'pet';
                if (!latestByKey[k] || String(m.timestamp || '') > String(latestByKey[k].timestamp || '')) {
                  latestByKey[k] = m;
                }
              });
              Object.entries(latestByKey).forEach(([k, m]) => {
                upsertPet(k, Number(m.lat), Number(m.lon), k);
              });
              return;
            }
          } catch {}
          ...
          try {
            const locRes = await fetch('/api/location/points?limit=50', { cache: 'no-store' });
            if (!locRes.ok) return;
            const data = await locRes.json();
            const points = Array.isArray(data.points) ? data.points : [];
            const latestByKey = {};
            points.forEach(p => {
              const k = p.userId || p.deviceId || 'pet';
              if (!latestByKey[k] || String(p.timestamp || '') > String(latestByKey[k].timestamp || '')) {
                latestByKey[k] = p;
              }
            });
            Object.entries(latestByKey).forEach(([k, p]) => {
              upsertPet(k, Number(p.lat), Number(p.lon), k);
            });
          } catch {}
        }
        ...
        return {
          ...
          setActive(next) {
            const shouldBeActive = Boolean(next);
      ...
      btn3d?.addEventListener('click', async (e) => {
        if (!map3dCanvas) return;
        if (!map3dEnabled) {
          const ok = await ensureThreeLoaded();
          if (!ok) {
            alert('3D engine failed to load. Please check your network and try again.');
            return;
          }
          map3dState = map3dState || create3dMap();
          map3dState?.setActive?.(true);
          setMap3dEnabled(true);
          // If enabled, allow quick calibration via prompt (course demo friendly).
          // Format: minLat,maxLat,minLon,maxLon
          if (map3dState?.getBounds) {
            const b = map3dState.getBounds();
            const hint = `${b.minLat},${b.maxLat},${b.minLon},${b.maxLon}`;
            // Hold Alt/Option while clicking to edit bounds.
            if (e.altKey) {
              const input = prompt('Set map bounds: minLat,maxLat,minLon,maxLon', hint);
              if (input) {
                const parts = input.split(',').map(s => Number(s.trim()));
                if (parts.length === 4 && parts.every(Number.isFinite) && parts[0] < parts[1] && parts[2] < parts[3]) {
                  map3dState.setBounds({ minLat: parts[0], maxLat: parts[1], minLon: parts[2], maxLon: parts[3] });
                } else {
      ...
      aiCollarSyncBtn?.addEventListener('click', async () => {
        await ensureLatestCollarSnapshot(true);
      });
      document.querySelectorAll('[data-module-action]').forEach((btn) => {
        btn.addEventListener('click', () => {
          const action = btn.getAttribute('data-module-action');
          testReservedModuleEndpoint(action);
        });
      });
      setSelectedAIService('diagnosis');
      resetDiagnosisUI();
      renderCollarSnapshotPreview();
      ensureLatestCollarSnapshot();
      ...
      async function handleDiagnosisRun() {
        ...
        try {
          await ensureLatestCollarSnapshot();
          const symptomsWithCollar = mergeContextWithCollar(symptoms || 'No extra symptom details provided.');
          if (diagResult) {
            diagResult.innerHTML = `
      ...
      async function handleTextServiceRun() {
        ...
        await ensureLatestCollarSnapshot();
        const rawContext = (textServiceInput?.value || '').trim() || buildProfileContext(user);
        const context = mergeContextWithCollar(rawContext);
        if (!context) {
    ...
    function initTabs() {
      ...
      const tabPages = {
        ...
        collar: document.getElementById('tab-collar'),
      };
    ...
    // --- Collar / vitals ---
    let collarInitialized = false;
    function initCollar() {
      if (collarInitialized) return;
      collarInitialized = true;

      const hrEl = document.getElementById('collar-hr');
      const dbEl = document.getElementById('collar-db');
      const tsEl = document.getElementById('collar-ts');
      const qualityEl = document.getElementById('collar-quality');
      const locEl = document.getElementById('collar-loc');
      const locSourceEl = document.getElementById('collar-loc-source');
      const logEl = document.getElementById('collar-log');
      const emptyEl = document.getElementById('collar-empty');
      const refreshBtn = document.getElementById('btn-collar-refresh');
      ...
      async function refresh() {
        ...
        const userKey = user?.username || user?.id || '';
        const url = userKey
          ? `/api/health/measurements?userId=${encodeURIComponent(userKey)}&limit=10`
          : `/api/health/measurements?limit=10`;
        try {
          const res = await fetch(url, { cache: 'no-store' });
          if (!res.ok) throw new Error(`HTTP ${res.status}`);
          const data = await res.json();
          const items = Array.isArray(data.items) ? data.items.slice().reverse() : [];
          latestCollarSnapshot = items[0] || null;
          renderCollarSnapshotPreview();
          if (!items.length) {
            emptyEl?.classList.remove('hidden');
          ...
          hrEl.textContent = Number.isFinite(latest.heartRateBpm) ? String(latest.heartRateBpm) : '--';
          dbEl.textContent = Number.isFinite(latest.soundLevelDb) ? String(latest.soundLevelDb) : '--';
          tsEl.textContent = fmtTs(latest.timestamp || latest.receivedAt);
          qualityEl.textContent = latest.quality !== null && latest.quality !== undefined ? String(latest.quality) : '--';

          if (Number.isFinite(latest.lat) && Number.isFinite(latest.lon)) {
            locEl.textContent = `${Number(latest.lat).toFixed(6)}, ${Number(latest.lon).toFixed(6)} (±${Number(latest.locationAccuracy || 0).toFixed(1)}m)`;
            locSourceEl.textContent = latest.metadata?.locationSource || '--';
          } else {
            locEl.textContent = '--';
            locSourceEl.textContent = '--';
          }
          ...
          if (logEl) {
            logEl.innerHTML = items.slice(0, 10).map(m => {
              const hr = Number.isFinite(m.heartRateBpm) ? `${m.heartRateBpm} bpm` : '-- bpm';
              const db = Number.isFinite(m.soundLevelDb) ? `${m.soundLevelDb} dB` : '-- dB';
                  const bat = Number.isFinite(m.batteryPct) ? `${m.batteryPct}%` : '--%';
                  const steps = Number.isFinite(m.steps) ? `${m.steps}` : '--';
                  const temp = Number.isFinite(m.tempC) ? `${m.tempC}°C` : '--°C';
                  const accel = Number.isFinite(m.accelPeak) ? `${m.accelPeak}` : '--';
                  const act = m.activity ? String(m.activity) : '--';
              const t = fmtTs(m.timestamp || m.receivedAt);
              const loc = (Number.isFinite(m.lat) && Number.isFinite(m.lon)) ? `${Number(m.lat).toFixed(5)}, ${Number(m.lon).toFixed(5)}` : '--';
              return `<div class="flex items-center justify-between bg-white/70 border border-white/70 rounded-xl px-3 py-2">
                <div class="min-w-0">
                  <p class="font-semibold text-[11px] text-slate-900">${t}</p>
                      <p class="text-[10px] text-gray-600">HR ${hr} · Sound ${db} · Bat ${bat} · Steps ${steps} · Temp ${temp} · Accel ${accel} · Act ${act} · Loc ${loc}</p>
                </div>
                <span class="text-[10px] text-primary font-semibold">${m.deviceId || 'device'}</span>
              </div>`;
            }).join('');
          }
        } catch (err) {
          if (logEl) logEl.innerHTML = `<p class="text-[11px] text-red-500">Load failed: ${err.message || String(err)}</p>`;
          emptyEl?.classList.add('hidden');
        }
      }
    ...
    const MONITOR_API_TOKEN = typeof import.meta !== 'undefined' ? import.meta.env?.VITE_MONITOR_API_TOKEN || '' : '';
    ...
    let latestCollarSnapshot = null;
    const RESERVED_MODULE_ENDPOINTS = {
      alerts: '/api/alerts/latest',
      trajectory: '/api/location/points?limit=20',
      nfc: '/api/nfc/public?userId=u1&petId=p1',
      device: '/api/device/status'
    };

    function formatCollarMetric(value, suffix = '') {
      return Number.isFinite(value) ? `${value}${suffix}` : '--';
    }

    function renderCollarSnapshotPreview() {
      if (!aiCollarContextPreviewEl) return;
      const context = buildCollarContext();
      aiCollarContextPreviewEl.textContent = context || 'No collar measurements yet. Open Collar tab or click Sync.';
    }

    async function ensureLatestCollarSnapshot(force = false) {
      if (latestCollarSnapshot && !force) return latestCollarSnapshot;
      const user = getCurrentUser?.() || null;
      const userKey = user?.username || user?.id || '';
      const url = userKey
        ? `/api/health/measurements?userId=${encodeURIComponent(userKey)}&limit=1`
        : `/api/health/measurements?limit=1`;
      try {
        const res = await fetch(url, { cache: 'no-store' });
        if (!res.ok) return latestCollarSnapshot;
        const data = await res.json();
        const items = Array.isArray(data.items) ? data.items : [];
        latestCollarSnapshot = items[0] || null;
      } catch {
        // Keep previous snapshot when network/server is unstable.
      }
      renderCollarSnapshotPreview();
      return latestCollarSnapshot;
    }

    function buildCollarContext() {
      const m = latestCollarSnapshot;
      if (!m) return '';
      const ts = m.timestamp || m.receivedAt || '--';
      const loc = (Number.isFinite(m.lat) && Number.isFinite(m.lon))
        ? `${Number(m.lat).toFixed(5)}, ${Number(m.lon).toFixed(5)}`
        : '--';
      return [
        'Collar telemetry (latest):',
        `- Heart rate: ${formatCollarMetric(m.heartRateBpm, ' bpm')}`,
        `- Sound level: ${formatCollarMetric(m.soundLevelDb, ' dB')}`,
        `- Battery: ${formatCollarMetric(m.batteryPct, '%')}`,
        `- Steps: ${formatCollarMetric(m.steps)}`,
        `- Temperature: ${formatCollarMetric(m.tempC, ' C')}`,
        `- Accel peak: ${formatCollarMetric(m.accelPeak)}`,
        `- Activity: ${m.activity ? String(m.activity) : '--'}`,
        `- Location: ${loc}`,
        `- Source: ${m.metadata?.locationSource || '--'}`,
        `- Captured at: ${ts}`
      ].join('\n');
    }

    function mergeContextWithCollar(baseContext) {
      const collarContext = buildCollarContext();
      if (!collarContext) return baseContext;
      return `${baseContext}\n\n${collarContext}`;
    }

    async function testReservedModuleEndpoint(action) {
      const url = RESERVED_MODULE_ENDPOINTS[action];
      if (!url) return;
      if (aiModuleStatusEl) aiModuleStatusEl.textContent = `Testing ${action} endpoint...`;
      try {
        const res = await fetch(url, { cache: 'no-store' });
        const status = `${res.status} ${res.ok ? 'OK' : 'ERROR'}`;
        if (aiModuleStatusEl) {
          aiModuleStatusEl.textContent = `${action} -> ${url} (${status})`;
        }
      } catch (err) {
        if (aiModuleStatusEl) {
          aiModuleStatusEl.textContent = `${action} -> ${url} failed: ${err.message || String(err)}`;
        }
      }
    }
    ...
    async function sendMonitoringPayload(payload = {}) {
      try {
        ...
        if (MONITOR_API_TOKEN) headers.Authorization = `Bearer ${MONITOR_API_TOKEN}`;
