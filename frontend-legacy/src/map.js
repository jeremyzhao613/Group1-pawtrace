  ...
  class PawMapController {
    ...
    setLink(url) {
      ...
      linkButton.onclick = () => {
        ...
        // Keep the button always "openable" for demo completeness.
        alert('This demo spot has no external link yet. Open the “API Console” tab to explore endpoints.');
      };
    }
    ...
    disableLinkButton() {
      const linkButton = this.cardElements.linkButton;
      if (!linkButton) return;
      // keep callable; do not hard-disable buttons in demo UI
      linkButton.disabled = false;
      linkButton.classList.remove('opacity-40', 'cursor-not-allowed');
      linkButton.onclick = () => {
        alert('This action is not available yet. Open the “API Console” tab for available actions.');
      };
    }
